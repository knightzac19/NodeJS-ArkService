...
module.exports.getSQData = function(cb) {
    ...
    sq.open(settings.sourcequery.host, settings.sourcequery.port,function(err){
		console.log(err);
		cb();
		return;
	});
    ...
    sq.close(function() {
		...
		//arkdedicated.com seems to returning crap versions, but not sure of an alternative at the moment...
        http.request({
